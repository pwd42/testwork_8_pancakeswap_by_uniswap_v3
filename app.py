...
async def main():
    ...
    # ввод процент слиппажа
    slippage = set_slippage_by_input(logger)

    # инициализация клиент uniswap_v2
    swap_client = Pancakeswap(client, logger)
    # выполнение транзакции swap
    print("Выполнение транзакции swap ... ")
    await  swap_client.swap(name_code_token_in, name_code_token_out, amount_for_swap_in_wei, slippage)
