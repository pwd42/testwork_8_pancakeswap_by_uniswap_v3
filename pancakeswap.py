...
from config import PANCAKESWAP_CONTRACTS, PANCAKESWAP_QUOTER_V2_ABI, PANCAKESWAP_ROUTER_V3_ABI, TOKENS_PER_CHAIN, ZERO_ADDRESS
...
class Pancakeswap:
    def __init__(self, client: Client, logger = None):
        ...
        self.quoter_contract : AsyncContract = self.client.get_contract(
            contract_address=PANCAKESWAP_CONTRACTS[self.client.chain_name_code]['quoter'],
            abi=PANCAKESWAP_QUOTER_V2_ABI
        )
        self.router_contract : AsyncContract = self.client.get_contract(
            contract_address=PANCAKESWAP_CONTRACTS[self.client.chain_name_code]['router'],
            abi=PANCAKESWAP_ROUTER_V3_ABI
        )
    ...
    async def get_min_amount_out(self, path : str, amount_in_wei: int, slippage : float):
        response_quote_exact_input = await self.quoter_contract.functions.quoteExactInput(
            path,
            amount_in_wei
        ).call()
        min_amount_out = response_quote_exact_input[0]
        self.logger.info(f"min_amount_out - {min_amount_out}")
        self.logger.info(f"min_amount_out with slippage - {int(min_amount_out * (1 - slippage / 100))}")
        return int(min_amount_out * (1 - slippage / 100))

    async def swap(self, from_token_name: str, to_token_name: str, amount_in_wei: int, slippage : float):

        path = self.get_path(from_token_name, to_token_name)
        deadline = int(time.time() + 1200)
        min_amount_out_in_wei = await self.get_min_amount_out(path, amount_in_wei, slippage)
